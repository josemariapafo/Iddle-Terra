...
namespace Terra.Data.Catalogos
{
    ...
    /// - Multiplicador coste 1.10 en eras 1-3 (antes 1.12)
    /// - Costes Era 1 rebajados para primera mejora en <10s
    /// </summary>
    public static class CatalogoMejoras
    {
        public static DefinicionMejora[] Crear()
        {
            ...
            mejoras[i++] = new DefinicionMejora("atm_01","Gases primordiales",    "H2, CH4 y NH3 emergen del interior",        TipoPilar.Atmosfera, 0, 5,        0.3,   1, null, TipoPilar.Atmosfera, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("atm_02","Vapor de agua",         "La evaporacion crea las primeras nubes",     TipoPilar.Atmosfera, 0, 50,       1.5,   1, new[]{"atm_01"}, TipoPilar.Atmosfera, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("atm_03","CO2 atmosferico",       "El efecto invernadero calienta el planeta",  TipoPilar.Atmosfera, 0, 300,      7.5,   2, new[]{"atm_02"}, TipoPilar.Atmosfera, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("atm_04","Nitrogeno estable",     "El N2 domina la composicion atmosferica",    TipoPilar.Atmosfera, 0, 2_000,    36,    2, new[]{"atm_03"}, TipoPilar.Atmosfera, 0, 1.10);
            ...
            // Zona 1 — Nubes
            mejoras[i++] = new DefinicionMejora("atm_08","Condensacion",          "Las primeras gotas de lluvia caen",           TipoPilar.Atmosfera, 1, 120,      3,     2, new[]{"atm_02"}, TipoPilar.Atmosfera, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("atm_09","Nubes cumulos",         "Masas de vapor cubren el cielo",              TipoPilar.Atmosfera, 1, 900,      15,    2, new[]{"atm_08"}, TipoPilar.Atmosfera, 0, 1.10);
            ...
            // ── OCEANOS ───────────────────────────────────────────────────
            // Zona 0 — Superficie
            mejoras[i++] = new DefinicionMejora("oc_01","Mares poco profundos",   "Las primeras cuencas oceanicas se llenan",    TipoPilar.Oceanos, 0, 6,         0.45,  1, null, TipoPilar.Oceanos, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("oc_02","Arrecifes primitivos",   "Estructuras de carbonato crecen en el mar",   TipoPilar.Oceanos, 0, 65,        1.8,   1, new[]{"oc_01"}, TipoPilar.Oceanos, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("oc_03","Plancton",               "Microorganismos transforman la quimica marina",TipoPilar.Oceanos,0, 450,       9,     2, new[]{"oc_02"}, TipoPilar.Oceanos, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("oc_04","Corrientes superficiales","El viento arrastra masas de agua calidas",   TipoPilar.Oceanos, 0, 3_200,     45,    2, new[]{"oc_03"}, TipoPilar.Oceanos, 0, 1.10);
            ...
            // Zona 1 — Zona media
            mejoras[i++] = new DefinicionMejora("oc_08","Corrientes oceanicas",   "El calor se distribuye por todo el oceano",   TipoPilar.Oceanos, 1, 190,       3.6,   2, new[]{"oc_02"}, TipoPilar.Oceanos, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("oc_09","Termoclina",             "Capa que separa agua calida de fria",          TipoPilar.Oceanos, 1, 1_300,     18,    2, new[]{"oc_08"}, TipoPilar.Oceanos, 0, 1.10);
            ...
            // ── TIERRA ────────────────────────────────────────────────────
            // Zona 0 — Superficie
            mejoras[i++] = new DefinicionMejora("ti_01","Roca basaltica",         "La corteza oceanica primitiva emerge",         TipoPilar.Tierra, 0, 6,         0.36,  1, null, TipoPilar.Tierra, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("ti_02","Sedimentos",             "Capas de material se acumulan en cuencas",     TipoPilar.Tierra, 0, 58,        1.65,  1, new[]{"ti_01"}, TipoPilar.Tierra, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("ti_03","Suelo primitivo",        "Weathering crea las primeras tierras fertiles",TipoPilar.Tierra, 0, 380,       8.4,   2, new[]{"ti_02"}, TipoPilar.Tierra, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("ti_04","Minerales complejos",    "Silicatos y carbonatos diversifican el suelo", TipoPilar.Tierra, 0, 2_600,     42,    2, new[]{"ti_03"}, TipoPilar.Tierra, 0, 1.10);
            ...
            // Zona 1 — Corteza
            mejoras[i++] = new DefinicionMejora("ti_08","Placas tectonicas",      "La corteza se fragmenta en placas moviles",    TipoPilar.Tierra, 1, 160,       3.3,   2, new[]{"ti_02"}, TipoPilar.Tierra, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("ti_09","Cadenas montanosas",     "La colision de placas eleva cordilleras",      TipoPilar.Tierra, 1, 1_150,     16.5,  2, new[]{"ti_08"}, TipoPilar.Tierra, 0, 1.10);
            ...
            // ── VIDA ─────────────────────────────────────────────────────
            // Zona 0 — Microvida
            mejoras[i++] = new DefinicionMejora("vi_01","Bacteria primordial",    "Las primeras moleculas autorreplicantes",      TipoPilar.Vida, 0, 5,          0.3,   1, null, TipoPilar.Vida, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("vi_02","Arqueobacterias",        "Vida extremofila en condiciones inhospitas",   TipoPilar.Vida, 0, 48,         1.5,   1, new[]{"vi_01"}, TipoPilar.Vida, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("vi_03","Celulas eucariotas",     "Un nucleo protege el material genetico",       TipoPilar.Vida, 0, 320,        7.5,   2, new[]{"vi_02"}, TipoPilar.Vida, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("vi_04","Organismos multicel.",   "Celulas que cooperan forman un ser mayor",     TipoPilar.Vida, 0, 2_250,      37.5,  2, new[]{"vi_03"}, TipoPilar.Vida, 0, 1.10);
            ...
            mejoras[i++] = new DefinicionMejora("vi_08","Algas fotosinteticas",   "La luz solar se convierte en energia vital",   TipoPilar.Vida, 1, 128,        3,     2, new[]{"vi_03"}, TipoPilar.Vida, 0, 1.10);
            mejoras[i++] = new DefinicionMejora("vi_09","Hongos",                 "Descomponedores que reciclan nutrientes",      TipoPilar.Vida, 1, 950,        15,    2, new[]{"vi_08"}, TipoPilar.Vida, 0, 1.10);
